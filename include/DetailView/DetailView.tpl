	...
	<tr>
	    ...
	    {{/if}}
			...
			<td width='{{$def.templateMeta.widths[$smarty.foreach.colIteration.index].field}}%' {{if $colData.colspan}}colspan='{{$colData.colspan}}'{{/if}} {{if isset($fields[$colData.field.name].type) && $fields[$colData.field.name].type == 'phone'}}class="phone"{{/if}}>
				<div style="width: 100%; min-height: 10px;" ondblclick="EDV.show_edit('{{$fields[$colData.field.name].name}}','{{$fields[$colData.field.name].type}}')" class="div_value" id="{{$fields[$colData.field.name].name}}_detailblock" target_id="{{$fields[$colData.field.name].name}}">
				...
				{{if !$colData.field.disable_edit}} 																										{{if $module == "Accounts" || $module == "Contacts"}}
				{{if !empty($colData.field.name)}}
					{if !isset($fields.{{$colData.field.name}}.acl) || (isset($fields.{{$colData.field.name}}.acl) && $fields.{{$colData.field.name}}.acl > 1) }
				{{/if}}
				
				<div class='edit_block' style="display: none;" id='{{$fields[$colData.field.name].name}}_editblock'>
						{{if ($fields[$colData.field.name].required && !isset($colData.field.displayParams.required)) ||  (isset($colData.field.displayParams.required) && $colData.field.displayParams.required && $fields[$colData.field.name].required !== false)}}
							<input type='hidden' id='{{$fields[$colData.field.name].name}}_required' value='true'>
						{{else}}
							<input type='hidden' id='{{$fields[$colData.field.name].name}}_required' value=''>
						{{/if}}

						{{if $fields[$colData.field.name] && $fields[$colData.field.name].type == 'relate'}}
							<input type='hidden' id='{{$fields[$colData.field.name].name}}_module' value='{{$fields[$colData.field.name].module}}'>
							<input type='hidden' id='{{$fields[$colData.field.name].name}}_id_name' value='{{$fields[$colData.field.name].id_name}}'>
						{{/if}}					
						
						{{if $fields[$colData.field.name] && $fields[$colData.field.name].name == 'email1'}}
							<input type='text' id='{{$fields[$colData.field.name].name}}' value=''>	
						{{elseif $fields[$colData.field.name] && $fields[$colData.field.name].name == 'duration_hours'}}
						...
						{{elseif $module == "Calls" && $fields[$colData.field.name] && $fields[$colData.field.name].name == 'direction'}}
							{{sugar_field parentFieldArray='fields' tabindex=$colData.field.tabindex vardef=$fields.direction displayType='EditView' displayParams=$colData.direction.displayParams formName=$form_name}}&nbsp;
							{{sugar_field parentFieldArray='fields' tabindex=$colData.field.tabindex vardef=$fields.status displayType='EditView' displayParams=$colData.status.displayParams formName=$form_name}}&nbsp;
						{{elseif $fields[$colData.field.name] && $fields[$colData.field.name].type == 'fullname'}}
							...
							<button type='button' id='{{$fields[$colData.field.name].name}}'>{$APP.LBL_EDV_SAVE}</button>													
						{{elseif $fields[$colData.field.name] && $fields[$colData.field.name].name != 'date_entered' && $fields[$colData.field.name].name != 'date_modified'}}
							{counter name="panelFieldCount"}
