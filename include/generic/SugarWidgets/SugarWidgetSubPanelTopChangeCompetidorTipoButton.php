...
class SugarWidgetSubPanelTopChangeCompetidorTipoButton extends SugarWidgetSubPanelTopButton
{
    private $formato = "";   

    //button_properties is a collection of properties associated with the widget_class definition. layoutmanager
    function SugarWidgetSubPanelTopChangeCompetidorTipoButton($button_properties=array())
    {
    ...
    {
        return $GLOBALS['app_strings']['LBL_CHANGE_TIPOCOMPETI_BUTTON_LABEL'];
    }
    ...
    function display(&$widget_data)
    {
        ...
        $bot='<input type="button" name="' . $this->getWidgetId() . '" id="' . $this->getWidgetId() . '" class="button"' . "\n"
                . ' title="' . $this->value." - ".$GLOBALS['app_list_strings']['lst_tipo_competidor'][$tipoComp] . '"'
            . ' value="' . $this->value."-".$GLOBALS['app_list_strings']['lst_tipo_competidor'][$tipoComp] . "\"\n"
            . " onclick='cambiarCompetidor("."\"".$tipoComp."\"".");' />\n";
    ...
    */
    ...
    {
       return translate('LBL_CHANGE_TIPOCOMPETI_BUTTON_TITLE');
    }
