...
class OutboundEmail {
	...
	function getUserMailerSettings(&$user, $mailer_id='', $ieId='') {
		...
		$q = "SELECT id FROM outbound_email WHERE user_id = '{$user->id}' {$mailer} order by name";
		$r = $this->db->query($q);
		$a = $this->db->fetchByAssoc($r);
	...
	function getInboundMailerSettings($user, $mailer_id='', $ieId='') {
		...
		$q = "SELECT id FROM outbound_email WHERE {$mailer} order by name";
		$r = $this->db->query($q);
		$a = $this->db->fetchByAssoc($r);
	...
	function isAllowUserAccessToSystemDefaultOutbound()
	{
	    ...
	    $q = "SELECT id FROM outbound_email WHERE type = 'system' order by name";
		$r = $this->db->query($q);
		$a = $this->db->fetchByAssoc($r);
		if (!empty($a)) {
