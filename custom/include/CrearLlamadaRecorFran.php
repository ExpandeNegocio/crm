<?php
    ...
    $query=$query." UNION ";
    $query=$query." select id from ";
    $query=$query." expan_franquicia where id not in (select distinct parent_id from calls where parent_type='Expan_Franquicia' ";
    $query=$query." AND TIMESTAMPDIFF (day, DATE(date_start), curdate())<15 AND deleted=0) and tipo_cuenta=2 and deleted=0;  ";
    ...
    while ($row = $db -> fetchByAssoc($result)) {
        ...
        $fran -> retrieve($idFran);
          ...
          $fran -> creaLlamadaRecor("[AUT]Recordatorio", "FRANOtra");
