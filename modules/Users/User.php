...
EOQ;
   ...
   function bean_implements($interface) {
	...
	function get_list_view_data() {
        ...
        /*$user_fields['DIR_CUENTA'] = $this->numCuentasDir();
        $user_fields['DIR_CONSULTORIA'] = $this->numDirConsultoria();        
        $user_fields['TOTAL_CUENTAS'] = $user_fields['DIR_CUENTA']+$user_fields['DIR_CONSULTORIA'];
        $user_fields['NUM_POSITIVAS'] = $this->numOpPositivas();
        $user_fields['NUM_ABIERTAS'] = $this->numOpAbiertas();*/

        $GLOBALS['log'] -> info('[ExpandeNegocio][USERS Class][numCuentasDir]Dir_cuenta-'.$user_fields['DIR_CUENTA']);
...
$fromaddr = $a['value'];
   ...
   {	//call parent method, specifying for array to be returned
    ...
    public function isPrimaryEmail($email)
    {
     ...
     public function numCuentasDir(){
        
      $db = DBManagerFactory::getInstance();  
        
        $query = "SELECT count(1) cont FROM expan_gestionsolicitudes g, expan_franquicia f ";
        $query = $query. "WHERE g.franquicia=f.id and f.chk_c2=1 and f.assigned_user_id ='".$this->id."'";
                                       
        $result = $db -> query($query, true);
        
        $GLOBALS['log'] -> info('[ExpandeNegocio][USERS Class][numCuentasDir]Consulta-'.$query);
        ...
        $GLOBALS['log'] -> info('[ExpandeNegocio][USERS Class][numCuentasDir]NumPositivas-'.$cont);
        
        return $cont;
    }
     
    public function numDirConsultoria(){
        
       $db = DBManagerFactory::getInstance();  
        
        $query = "SELECT count(1) cont FROM expan_gestionsolicitudes g, expan_franquicia f ";
        $query = $query. "WHERE g.franquicia=f.id and f.chk_c2=0 and f.assigned_user_id ='".$this->id."'";
    ...
    public function numOpPositivas(){
        
        $db = DBManagerFactory::getInstance();  
        
        $query = "SELECT count(1) cont FROM expan_gestionsolicitudes g, expan_franquicia f ";
        $query = $query. "WHERE g.franquicia=f.id and estado_sol='".Expan_GestionSolicitudes::ESTADO_POSITIVO."' AND f.assigned_user_id ='".$this->id."'";
    ...
    public function numOpAbiertas(){
        
        $db = DBManagerFactory::getInstance();  
        
        $query = "SELECT count(1) cont FROM expan_gestionsolicitudes g, expan_franquicia f ";
        $query = $query. "WHERE g.franquicia=f.id and (estado_sol='".Expan_GestionSolicitudes::ESTADO_NO_ATENDIDO;
        $query = $query. "' OR estado_sol='".Expan_GestionSolicitudes::ESTADO_EN_CURSO."') AND f.assigned_user_id ='".$this->id."'";
