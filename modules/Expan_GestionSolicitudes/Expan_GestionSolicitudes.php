...
' . $fecha . ' : ' . $texto;        //NO AÑADIR ESPACIOS AL INICIO DE LA LINEA
    ...
    public function crearTablaEntregaCuentaPrecontrato($esAdmin)
    {
      ...
      $GLOBALS['log']->info('[ExpandeNegocio][crearTablaEntregaCuentaPrecontrato] Query - ' . $query);
      ...
      while ($row = $db->fetchByAssoc($result)) {
        foreach ($campos as $key => $value) {
          ...
          if ($key == 'ImporteEnt') {
            ...
            $valor = $this->provincia_apertura_pre;
          }
          $tabla = $tabla . "<tr>";
          ...
          $GLOBALS['log']->info('[ExpandeNegocio][crearTablaEntregaCuentaPrecontrato] Query - ' . $value . '-' . $valor);
        }
      }
    ...
    public function activarEntPrevia()
    {
      $solicitud = $this->GetSolicitud();
      $solicitud->chk_entrevista_previa_EN=1;

      $now = TimeDate::getInstance()->getNow();
      $now = $now->setTime($now->hour, $now->min, "00")->asDb();
      $solicitud->f_entrevista_previa_EN=$now;
      ...
      foreach ($solicitud->expan_solicitud_expan_gestionsolicitudes_1->getBeans() as $gestionHermana) {
        if ($gestionHermana -> id != $bean -> id) {
          ...
          $gestionHermana ->chk_entrevista_previa=1;
          $gestionHermana ->ignore_update_c = true;
          $gestionHermana->save();
        }
      }
