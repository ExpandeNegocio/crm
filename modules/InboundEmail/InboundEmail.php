...
//			_pp($UIDLs);_pp($cacheUIDLs);
	...
	function deletePersonalEmailAccount($id, $user_name) {
		$q = "SELECT ie.id FROM inbound_email ie LEFT JOIN users u ON ie.group_id = u.id WHERE u.user_name = '{$user_name}' order by ie.name";
		$r = $this->db->query($q, true);
	...
	*/
	...
	function importOneEmail($msgNo, $uid, $forDisplay=false, $clean_email=true) {
        ...
        $GLOBALS['log']->info('[ExandeNegocio][ImportoMensaje] Entra a importar 1: ');
        ...
        $GLOBALS['log']->info('[ExandeNegocio][ImportoMensaje] Entra a importar 2: ');

		// handle messages deleted on server
		if(empty($header)) {
		    
            $GLOBALS['log']->info('[ExandeNegocio][ImportoMensaje] Entra a importar 3: ');
            
			if(!isset($this->email) || empty($this->email)) {
				$this->email = new Email();
			}

            $GLOBALS['log']->info('[ExandeNegocio][ImportoMensaje] Entra a importar 4: ');
            ...
            $GLOBALS['log']->info('[ExandeNegocio][ImportoMensaje] Entra a importar 5: ');
...
eoq;
	...
	function retrieveByGroupId($groupId) {
		$q = 'SELECT id FROM inbound_email WHERE group_id = \''.$groupId.'\' AND deleted = 0 AND status = \'Active\'  order by name';
		$r = $this->db->query($q, true);
...
//			_pp("count elements in sort[sort]: ".count($sorts[$sort]));
    ...
    */
    ...
    protected function importMailboxMessages($protocol, $mailbox = null)
    {
        ...
        $GLOBALS['log']->info("[ExpandeNegocio][ImportoMensaje] Entro");   
        switch ($protocol) {
            case 'pop3':
        ...
        foreach ($msgNumbers as $msgNumber) {
            ...
            $GLOBALS['log']->info('[ExpandeNegocio][ImportoMensaje] Importing message no: ' . $msgNumber);
            $this->importOneEmail($msgNumber, $uid, false, false);
            $GLOBALS['log']->info('[ExandeNegocio][ImportoMensaje] Importado message no: ' . $msgNumber);
        }
    }
